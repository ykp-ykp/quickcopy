...
| 后台驻留 | 最小化 / 关闭主窗口后隐藏到系统托盘，不退出；托盘菜单可「显示主界面 / 开机自启 / 退出」 |
| 开机自启 | 托盘菜单勾选「开机自启」即写入 HKCU Run 注册表（无需管理员权限），开机后自动驻留托盘 |
...
- 图标：`app.ico`（exe 文件图标）与 `app.png`（托盘 / 窗口 / 任务栏，已打进 exe）。
  想换图标直接替换这两个文件重新打包即可；也可用 `python make_icon.py`
  重新生成默认样式（需 `pip install pillow`）
...
7. **开机自启**：右键托盘图标 → 勾选「开机自启」，下次开机自动启动并驻留托盘；取消勾选即关闭。
8. **退出**：右键托盘图标 →「退出 QuickCopy」。
...
├─ QuickCopy.spec     # PyInstaller 配置（单文件/无控制台/图标/瘦身规则）
├─ make_icon.py       # 图标生成脚本（产出 app.png + app.ico，需 pillow）
├─ app.png / app.ico  # 应用图标（运行时 / exe 文件图标）

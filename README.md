...
| 主界面 | 只显示 Key 列表，单击复制 Value，双击编辑，右键弹出编辑/删除菜单，条目间有细分隔线 |
| 搜索过滤 | 主界面顶部搜索框，按 Key 不区分大小写模糊过滤，清空即恢复完整列表 |
| 最近置顶 | 复制某个 Key 后，该条目自动移到列表首位并持久化，常用内容永远在第一行 |
| 复制提示 | 复制成功弹出置顶、无边框、半透明磨砂 Toast，约 2 秒透明度淡出消失；若浮动面板展开中则 1 秒内淡出关闭 |
...
3. **复制**：单击列表中的 Key，Value 立即进入剪贴板，屏幕上方弹出「✅ 复制成功！」提示并淡出；被复制的条目自动置顶到第一行。
4. **编辑 / 删除**：选中条目后点底部按钮，或双击条目编辑、右键条目选择操作。
...
```
quickcopy/
├─ main.py            # 界面与交互（主窗口、浮动面板、Toast、托盘、鼠标轮询）
...
会验证数据层增删改查、剪贴板复制、浮动面板显示 / 延时隐藏 / 取消隐藏、Toast 自动消失等核心逻辑。
